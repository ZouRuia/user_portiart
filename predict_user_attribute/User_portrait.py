...
class UserProfileSVMModel(object):
    ...
    def __init__(self, data_jsonl, filepath_age, filepath_position, filepath_trade,
                 filepath_gender, word2vector_file_path, age200, gender200, trade200, input_number,
                 sentence_maxlen):
    ...
    def load_model(self):
        ...
        self.embedding_matrix, self.word2vector_dict, self.word2index_dict = self.trans_gensim_word2vec2tf_embedding()
    ...
    def input_deal(self, params):
        ...
        for i in range(len(data_list_list)):
            ...
            temp_text_list = []
            for j in range(10):
                c_text_list = [*title_list_list[i][j]["title"],
                               "<PADDING>", *x_list_list[i][j]["content"][:512]]
    ...
    def trans_gensim_word2vec2tf_embedding(self):
        ...
        word2vec_model = gensim.models.Word2Vec.load(
            self.word2vector_file_path)

        # 所有的词
        word_list = [word for word,
                     word_info in word2vec_model.wv.vocab.items()]
        ...
        embeddings_matrix[word2index_dict["<UNK>"]] = (
            1 / np.sqrt(len(word_list) + specical_word_count) * (2 * np.random.rand(word2vec_model.vector_size) - 1))
    ...
    def trans_multi_input_tokenize_data2npa(self, data_file_path, x_max_length, word2index_dict):
        ...
        with open(self.data_jsonl) as f:
            for line in f:
                ...
                x_list.append([[self.trans2index(self.word2index_dict, word)
                                for word in word_list] for word_list in text_tokenize_list])

      #  print("x_list[:1]:{}".format(x_list[:1]))

        if not x_max_length:
            #    x_max_length0 = np.max(np.array([len(v) for v in x_list]))
            x_max_length = int(np.max(np.percentile(
                np.array([len(v) for v in x_list]), 99.7)))
        ...
        for i in range(len(x_list)):
            x_list[i] = tf.keras.preprocessing.sequence.pad_sequences(
                x_list[i], maxlen=x_max_length, dtype=np.int32, truncating="post", padding='post', value=0)
    ...
    def data_trade_deal(self, x_npa):
        ...
        x_trade = x_trade.groupby(x_trade.index).filter(
    ...
    def data_mining(self, x_npa):
        ...
        age_x = age_x.groupby(age_x.index).filter(
            lambda x: float(x['rank']) == 1)
    ...
    def predict(self, params):
        ...
        for type_1 in type_list:
            if type_1 == "age":
                ...
                output_position = tf.keras.layers.Dense(
                    7, activation='softmax', use_bias=True, kernel_regularizer=tf.keras.regularizers.l2())(pre)
        ...
        for i in range(len(data_list_list)):
            ...
            dict_all["position"] = dic_position[result["result"]
                                                [0]['position'][i]]
    ...
    "data_list_list": [
        [
            ...
            },
            {
                "id": "349729",
                "title": "少量的词汇",
                "content": "少量的词汇可以自己用下面方法手动添加"
            },
            {
                "id": "347400",
                "title": "可调节",
                "content": "可调节单个词语的词频，使其能（或不能）被分出来"
            },
            {
                "id": "344255",
                "title": "待提取的文本",
                "content": "仅包括指定词性的词，默认值为空，即不筛选"
            },
            {
                "id": "340988",
                "title": "关键词提取",
            ...
            },
            {
                "id": "348764",
                "title": "关键词提取所使用停止词",
                "content": "文本语料库可以切换成自定义语料库的路径"
            },
            {
                "id": "3409358",
                "title": "词性标注",
                "content": "参数可指定内部使用的的原理后面再来说"
            },
            {
                "id": "3432587",
                "title": "尽管我们这里不更新",
...
a = UserProfileSVMModel(filepath_age=config_ini_dict["file"]["filepath_age"],
