...
class Number_Transform(object):
    def __init__(
        self, input_number, sentence_maxlen, age_output_data_path, word2vector_file_path
    ):
    ...
    def out_x_y(self):
        ...
        # word2vector_file_path = self.word2vector_file_path

        # 导入gensim的word2vector
        # embedding_matrix, word2vector_dict, word2index_dict = self.trans_gensim_word2vec2tf_embedding(
        ...
        x_npa, y_npa, tag2index_dict = self.trans_multi_input_tokenize_data2npa(
            self.age_output_data_path, self.sentence_maxlen, self.word2index_dict
        )
        #   class_weight_dict = {tag: np.sqrt(
        #     len(y_npa)/number) for tag, number in enumerate(np.bincount(y_npa))}
